...
class ConfigTestCase(unittest.TestCase):
    ...
    def test1(self):
        ...
        assert (
            cf["warc_name_pattern"]
            == "{prefix}-{timestamp}-{serial}-{pid}~{host}~{port}"
        )
        assert (
            cf["warc_name_pattern_upload"]
            == "{prefix}-{timestamp}-{serial}-{pid}~{host}~{port}"
        )
        assert (
    ...
    def testCustomNaming(self):
        ...
        assert cf.warc_name_pattern == "{prefix}-part-{serial}"
        assert cf.item_name_template == "CRAWL-{prefix}-{serial}-{lastserial}"
        assert cf.item_name_template_sh == "CRAWL-${prefix}-${serial}-${lastserial}"

    def testBadCustomNaming(self):
        cf = self._conf(TESTCONF_BAD_CUSTOM_NAMING)

        self.assertEqual(
            "{prefix}-part-{serial}", cf["warc_name_pattern"], "warc_name_pattern"
        )
        self.assertEqual(
            "CRAWL-{prefix}-{timestamp}-{serial}-{lastserial}",
            cf["item_name_template"],
            "item_name_template",
        )
