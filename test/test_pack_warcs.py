...
class PackWarcsTest(unittest.TestCase):
    def testStandardPack(self):
        ...
        wnames = (
            "WIDE-2010121200%02d00-%05d-2145~localhost~9443" % (n, n) for n in range(10)
        )
        ...
        self.assertEqual(
            0, rc, "pack-warcs.sh exit code, " "expected %d, got %d" % (0, rc)
        )

        assert not os.path.exists(
            os.path.join(ws.jobdir, "PACK.open")
        ...
        p = subprocess.Popen(
            [bin("s3-launch-transfers.sh"), ws.configpath, "1", "test"]
        )
        rc = p.wait()
        self.assertEqual(0, rc)
    ...
    def check_item_dir(self, ws, item_name, warcs_packed, total_size):
        ...
        self.assertEqual(
            1,
            len(lines),
            "number of lines in PACKED, " "expected %d, got %d" % (1, len(lines)),
        )
