...
def getdtprocesses(dtconf, excludes=[]):
    ...
    for pid in getpids(excludes):
        try:
            ...
            start_time = None
        cmdline = readfile(os.path.join("/proc", str(pid), "cmdline")).split("\0")
        ...
        if cmd == "s3-drain-job.sh":
            result.append(
                Storage(
                    p=Storage(pid=pid), cmdline=cmdline, o=getstdout(pid), st=start_time
                )
            )
...
class Series(object):
    ...
    def status(self):
        ...
        if not os.path.isfile(self.RETRY):
            if self.has_file("LAUNCH.open"):
                return "running"
            if self.has_file("MANIFEST.open"):
                return "running"
        return ""
...
class Project(object):
    ...
    def loadconfig(self, force=False):
        if force or self.is_config_updated():
            ...
            try:
                ...
                print(
                    "Aborting: invalid config %s: %s" % (self.config_fname, ex),
                    file=sys.stderr,
                )
                sys.exit(1)
    ...
    def configitems(self):
        return iter(self.configobj.items())
    ...
    def source(self):
        ...
        if os.path.isdir(srcdir):
            ...
            return Storage(
                exists=False, drainme=False, finishdrain=False, free=0, avail=0
            )
        ...
        if os.path.isdir(xferdir):
            for e in os.listdir(xferdir):
                ...
                if os.path.isdir(p):
                    s = Series(xferdir, e)
                    serieses.append(s)
        return sorted(serieses, lambda a, b: cmp(a.mtime, b.mtime), reverse=True)
    ...
    def start_drain_job(self):
        for step in ("pack", "manifest", "ingest", "clean"):
            ...
            if returncode != 0:
                print(
                    ("ERROR step %s failed with returncode %d" % (step, returncode)),
                    file=sys.stderr,
                )
                continue
...
class UpLoader:
    def __init__(self, configs, sleep=None, home=None, prefix=""):
        ...
        self.projects = [Project(i, config, self) for i, config in enumerate(configs)]
...
if __name__ == "__main__":
    ...
    opt.add_option(
        ...
        dest="interval",
        type="int",
        help="time in seconds to sleep between draining",
        default=None,
    )
    opt.add_option(
        "-L",
